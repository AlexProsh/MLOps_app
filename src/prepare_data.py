...
from ta.momentum import pvo_signal


def rolling_count(df, minutes, direction, cap=0.005):
    if direction == "up":
        df[f'y_{minutes}_{direction}'] = df["Close"].rolling(minutes * 60).apply(
            lambda x: ((x / df["Average"].shift(minutes * 60)) - 1 > cap).sum() / (minutes * 60)).shift(-minutes * 60)
    else:
        df[f'y_{minutes}_{direction}'] = df["Close"].rolling(minutes * 60).apply(
            lambda x: ((x / df["Average"].shift(minutes * 60)) - 1 < -cap).sum() / (minutes * 60)).shift(-minutes * 60)
    return df
...
def main(mode):
    for directory in tqdm(os.listdir(f"../data/raw/{mode}")):
        ...
        for file in tqdm(os.listdir(path_dir)):
            ...
            # Sort by time
            ds.sort_values('DateTime', inplace=True)
            # Add target (percentage of time when close price above current average price by 0.5% for next 10 minutes)
            rolling_count(ds, 10, "up")
            # Add target (percentage of time when close price below current average price by 0.5% for next 10 minute)
            rolling_count(ds, 10, "down")
            # Add target (percentage of time when close price above current average price by 1% for next 60 minutes)
            rolling_count(ds, 60, "up", cap=0.01)
            # Add target (percentage of time when close price below current average price by 1% for next 60 minute)
